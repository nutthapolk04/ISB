...
import { departmentToStudentLookup } from "@/lib/posCardLookup";
...
export function CardTapModal({ open, onOpenChange, currentMember, onSelect, scannedMember }: Props) {
    ...
    const lookup = async (q: string) => {
        ...
        try {
            ...
            try {
                const result = await api.get<DepartmentLookupResult>(
                    `/departments/by-card/${encodeURIComponent(trimmed)}`,
                );
                if (result) {
                    setFound(departmentToStudentLookup(result));
                    return;
                }
            } catch (e) {
                if (!(e instanceof ApiError && e.status === 404)) throw e;
