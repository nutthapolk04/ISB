...
export default function CardManagement() {
    ...
    const load = async () => {
        ...
        try {
            const [u, s, d, cardholders] = await Promise.all([
                ...
                api.get<{ items?: CardholderApiItem[] }>("/admin/cardholders?kind=other&page_size=500"),
            ]);
            setUsers(u);
            setStudents(s);
            setDepartments(d);
            // Customers in the "other" bucket (customer_kind='other') — the one
            // group neither /users-admin/ nor /users-admin/students returns. The
            // helper drops the users this endpoint also returns; see its module doc
            // for why that filter is a correctness requirement, not tidiness.
            setOtherCustomers(otherCustomerRowsFrom(cardholders.items));
        } catch (e) {
            toast({
    ...
    const boundCards: BoundCard[] = useMemo(
        () => buildBoundCards({ users, students, departments, otherCustomers }),
    ...
    const roleBadge = (c: BoundCard) => {
        if (c.kind === "customer") {
            const resolved = customerBadgeLabel(c.customerType, c.role);
            const label =
                resolved.source === "customerType" ? resolved.value
                    : resolved.source === "role" ? t(`admin.cards.role.${resolved.value}`, { defaultValue: resolved.value })
                        : t("admin.cards.roleStudent");
            return (
    ...
    const staffTypeBadge = (c: BoundCard) => {
        ...
        return (
            <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${isClassified
