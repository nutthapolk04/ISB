                            ...
                            <Popover open={productPickerOpen} onOpenChange={setProductPickerOpen}>
                                ...
                                <PopoverContent className="w-[--radix-popover-trigger-width] p-0" align="start">
                                    <Command
                                        filter={(value, search) => {
                                            if (value === "all") {
                                                return t("balanceFile.allProducts", "All products").toLowerCase().includes(search.toLowerCase()) ? 1 : 0;
                                            }
                                            const p = products.find((x) => String(x.id) === value);
                                            if (!p) return 0;
                                            const blob = `${p.product_code ?? ""} ${p.name}`.toLowerCase();
                                            return blob.includes(search.toLowerCase()) ? 1 : 0;
                                        }}
                                    >
                                        <CommandInput placeholder={t("balanceFile.searchPlaceholder", "Product code or name")} />
                                        <CommandList onWheel={(e) => e.stopPropagation()} className="max-h-72">
