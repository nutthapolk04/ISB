...
interface AuthContextValue {
    ...
    loginWithGoogleCredential: (credential: string) => Promise<{ success: boolean; error?: string; allRoles?: UserRole[] }>;
    logout: () => void;
...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    const loginWithGoogleCredential = async (
        ...
        try {
            const res = await fetch(`${API_BASE_URL}/auth/sso/google/id-token`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    return (
        <AuthContext.Provider value={{ user, isAuthenticated, login, loginWithMockSSO, loginWithGoogleCode, loginWithGoogleCredential, logout, hasRole, hasShopAccess, setActiveRole }}>
            {children}
