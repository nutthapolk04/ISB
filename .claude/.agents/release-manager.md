...
1. ตรวจสอบ **Pre-release Checklist**:
   ...
   - [ ] i18n: `frontend/src/locales/en.json` และ `th.json` มี key ตรงกันครบ
   - [ ] TypeScript: `npx tsc -p tsconfig.app.json --noEmit` ผ่าน (จาก `frontend/`)
   - [ ] Lint: `npm run lint` ผ่าน (จาก `frontend/`)
   - [ ] Build: `npm run build` สำเร็จ (จาก `frontend/`)
   - [ ] Backend: `bun test` ผ่าน (จาก `backend-bun/`)
