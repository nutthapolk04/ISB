...
- `backend-bun/.env.prod` or `.env.uat`
  - `ISB_PHOTO_DIR=/sftp/sftp-client/upload` (must exist on the host — profile photos + kiosk APKs)
...
`GET /api/v1/profile-photos/:filename` and `GET /api/v1/kiosk/releases/:filename` can read SFTP uploads.
